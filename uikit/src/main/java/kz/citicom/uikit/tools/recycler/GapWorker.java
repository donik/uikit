...
final class GapWorker implements Runnable {
    ...
    public void add(RecyclerView recyclerView) {
        if (RecyclerView.DEBUG && mRecyclerViews.contains(recyclerView)) {
            throw new IllegalStateException("RecyclerView already pushViewController in worker list!");
        }
    ...
    public void run() {
        try {
            ...
            if (latestFrameVsyncMs == 0) {
                // abort - either no views visible, or couldn't getPlayer last vsync for estimating next
                return;
            }
